...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $responseData = json_decode($verifyResponse);
    ...
    function logActivity($conn, $user_id, $activity_type, $activity_status, $role, $fname, $username) {
        $activity_details = "$fname ($role) with email $username logged in. Status: $activity_status";
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (password_verify($password, $hashed_password)) {
                ...
                logActivity($conn, $id, $activity_type, $activity_status, $role, $fname, $username);
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            logActivity($conn, 0, $activity_type, $activity_status, "Unknown", "Unknown", $username); // Log failed attempt with no user ID
            $_SESSION['swal_error'] = [
                'title' => 'Error!', 
                'text' => 'Invalid email or password.',
                'icon' => 'error'
...
<head>
  ...
  <link rel="icon" href="../img-icon/user.webp" type="image/png">
